...
def assembler():
	...
	# update data vhdl file
	dmem_file = 'dmem.vhd'
	data = [val[:-1] for val in dataConvertedArr[2:]]
	update_vhdl_mem_type(dmem_file, data)

	# update text vhdl file
	irmem_file = 'irmem.vhd'
	text = [val[:-1] for val in textConvertedArr[2:]]
	update_vhdl_mem_type(irmem_file, text)
...
def update_vhdl_mem_type(vhdl_file: str, values: list):
    # Convert values to VHDL array initialization string
    ...
    vhdl_text = re.sub(r"mem_type\s*:=\s*\([^;]*\)", f"mem_type := {new_values_str}", vhdl_text)

    # Write updated VHDL file
